...
class WxValidate {
  ...
  addMethod (name, method, message) {
    ...
    this.defaults.messages[name] = message !== undefined ?
      message :
  ...
  defaultMessage (param, rule) {
    let message = this.customMessage(param, rule) ||
      this.defaults.messages[rule.method]
  ...
  checkParam (param, rules, data) {
    ...
    // 缓存字段对应的值
    const value = data[param] !== null && data[param] !== undefined ?
      data[param] :
      ''
